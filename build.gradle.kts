...
plugins.withId("base") {
    tasks.named(BUILD_TASK_NAME).callSubprojectTasks(BUILD_TASK_NAME)
    ...
    tasks.register("release", DefaultTask::class.java).configure {
        ...
        project.tasks.findByName(BUILD_TASK_NAME)?.let {
            dependsOn(it)
            mustRunAfter(it)
        }
    }
...
private fun <T: Task> TaskProvider<T>.callSubprojectTasks(vararg dependOnTasks: String, configure: ((task: Task) -> Unit)? = null) {
    this.configure {
        if (configure != null) {
            configure(this)
        }
